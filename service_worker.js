...
async function sendMessageToTab(tabId, message) {
  ...
  async function injectContentScript(tabId) {
    try {
      ...
      if (results && results.length > 0) {
          // console.log(`SW: Content script injected successfully into tab ${tabId}. Result:`, results);
          // executeScript 的结果是一个数组，每个frame一个结果对象。
          // 如果没有错误，通常表示成功。
          return true;
      }
      // console.warn(`SW: Content script injection into tab ${tabId} completed but 'results' was empty or unexpected.`, results);
  ...
  chrome.action.onClicked.addListener(async (tab) => {
      ...
      if (tab.url && (tab.url.startsWith('https://x.com/') || tab.url.startsWith('https://twitter.com/'))) {
        ...
        if (!messageSent) {
          // console.log(`SW: Initial message send failed to tab ${tab.id}. Attempting to inject script.`);
          ...
          if (scriptInjected) {
            ...
            await new Promise(resolve => setTimeout(resolve, 100)); // 保留一个小的延迟
    ...
    chrome.runtime.onInstalled.addListener(async () => {
      ...
      try {
          ...
          console.error("SW: Error initializing storage on install:", error);
      }
    });  